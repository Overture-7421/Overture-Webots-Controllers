...
Supervisor* Supervisor::getSupervisorInstance() {
	if (cInstance) {
		if (!dynamic_cast<Supervisor*>(cInstance)) {
			cerr
					<< "A Robot instance is already present, it cannot be casted to Supervisor"
					<< endl;
			return NULL;
		}
...
void Supervisor::simulationRevert() {
	fprintf(stderr,
			"Supervisor::simulationRevert is deprecated, please use Supervisor::worldReload instead\n");
	wb_supervisor_world_reload();
}

void Supervisor::simulationPhysicsReset() {
	fprintf(stderr,
			"Supervisor::simulationPhysicsReset is deprecated, please use Supervisor::simulationResetPhysics instead\n");
...
void Supervisor::loadWorld(const std::string &file) {
	fprintf(stderr,
			"Supervisor::loadWorld is deprecated, please use Supervisor::worldLoad instead\n");
...
bool Supervisor::saveWorld() {
	fprintf(stderr,
			"Supervisor::saveWorld is deprecated, please use Supervisor::worldSave instead\n");
...
void Supervisor::startMovie(const string &file, int width, int height,
		int codec, int quality, int acceleration, bool caption) {
	fprintf(stderr,
			"Supervisor::startMovie is deprecated, please use Supervisor::movieStartRecording instead\n");
...
void Supervisor::stopMovie() {
	fprintf(stderr,
			"Supervisor::stopMovie is deprecated, please use Supervisor::movieStopRecording instead\n");
...
int Supervisor::getMovieStatus() {
	fprintf(stderr,
			"Supervisor::getMovieStatus is deprecated, please use Supervisor::movieGetStatus instead\n");
...
void Supervisor::setLabel(int id, const string &label, double xpos, double ypos,
		double size, int color, double transparency, const string &font) {
