...
Node* Node::getFromProtoDef(const std::string &name) const {
	WbNodeRef internalNodeRef = wb_supervisor_node_get_from_proto_def(nodeRef,
			name.c_str());
...
Field* Node::getField(const std::string &fieldName) const {
	WbFieldRef fieldRef = wb_supervisor_node_get_field(nodeRef,
			fieldName.c_str());
...
Field* Node::getProtoField(const std::string &fieldName) const {
	WbFieldRef fieldRef = wb_supervisor_node_get_proto_field(nodeRef,
			fieldName.c_str());
	return Field::findField(fieldRef);
}

Field* Node::getProtoFieldByIndex(const int index) const {
	WbFieldRef fieldRef = wb_supervisor_node_get_proto_field_by_index(nodeRef,
			index);
	return Field::findField(fieldRef);
}
