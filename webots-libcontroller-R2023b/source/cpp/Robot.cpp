...
Robot::Robot() {
	if (cInstance == NULL)
		...
		cerr << "Only one instance of the Robot class should be created"
				<< endl;
		wb_robot_step(0);
		wb_robot_cleanup();
		exit(-1);
...
string Robot::getData() const {
	fprintf(stderr,
			"Robot::getData is deprecated, please use Robot::getCustomData instead\n");
	return string(wb_robot_get_custom_data());
}

void Robot::setData(const std::string &data) {
	fprintf(stderr,
			"Robot::setData is deprecated, please use Robot::setCustomData instead\n");
	wb_robot_set_custom_data(data.c_str());
}
