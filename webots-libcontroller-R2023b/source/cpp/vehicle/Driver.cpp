...
Driver::Driver() {
	if (dInstance == NULL) {
		...
		std::cerr << "Only one instance of the Driver class should be created"
				<< std::endl;
		exit(-1);
	}
}
...
void Driver::setBrake(double brake) {
	assert(this);
	std::cerr
			<< "Warning: Deprecated 'setBrake' use 'setBrakeIntensity' instead."
			<< std::endl;
...
void Driver::setWipersMode(Driver::WiperMode mode) {
	assert(this);
	std::cerr
			<< "Warning: Deprecated 'setWipersMode' use 'setWiperMode' instead."
			<< std::endl;
	wbu_driver_set_wiper_mode(WbuDriverWiperMode(mode));
}
...
Driver::WiperMode Driver::getWipersMode() {
	assert(this);
	std::cerr
			<< "Warning: Deprecated 'getWipersMode' use 'getWiperMode' instead."
			<< std::endl;
	return WiperMode(wbu_driver_get_wiper_mode());
}
